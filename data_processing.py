...
class SepsisDataset(Dataset):
    def __init__(self, data_dir = None, is_train=True, window_size=12, device ='cpu'):
        self.device = device
        ...
        self.scaler = None
        self.flag = "training" if is_train else "validation"
    ...
    def setup(self):
        ...
        self.windows = [torch.nan_to_num(torch.Tensor(self.scaler.transform(w)))
                        for w in tqdm(self.windows, desc=f"Scaling {self.flag} data")]
        self.windows = torch.stack(self.windows, dim=0).to(self.device)    
        self.targets = torch.stack(self.targets, dim=0).to(self.device)
    ...
    def process_psv_file(self, file_path, window_size=10):
        ...
        # Handle case when time_series_data.shape[0] > 100
        # For comparison with TCN s2s architecture
        if 100 < time_series_data.shape[0]:
            time_series_data = time_series_data[:100]
...
if __name__ == '__main__':
    ...
    val_set.windows = [train_set.scaler.transform(w) 
